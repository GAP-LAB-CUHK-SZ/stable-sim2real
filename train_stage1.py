...
from data.dataset_train_stage1 import Sim2RealDataset
import torch
...
def get_argparse():
    ...
    parser.add_argument('--resume_path', default='pretrained_weights/init-sd21_vae_main.ckpt') # path to SD
    parser.add_argument('--data_path', default='dataset/lasa_depth') # path to lasa_depth (training data)
    parser.add_argument('--exp_name', default='stage1')
    parser.add_argument('--config_file', default='config/train_stage1.yaml')
...
def main(args):
    ...
    dataset = Sim2RealDataset(data_dir=args.data_path, resolution=args.resolution)
