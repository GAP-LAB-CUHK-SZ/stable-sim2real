...
class UnetModel_Guider(UNetModel):
    def forward(self, x, timesteps=None, context=None, y=None, guider=None, **kwargs):
        hs = []
...
class ControlLDMReferenceNet(LatentDiffusion):
    def __init__(self, control_stage_config, control_key, only_mid_control, prior_model=None, guider_config = None, *args, **kwargs):
        ...
        self.dino_guider = instantiate_from_config(guider_config)
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        hint_z = self.anything_prior(control).detach()
        hint_z = einops.rearrange(hint_z, 'b h w c -> b c h w')

        return x, dict(c_crossattn=[c], c_concat=[control_z], prior_out=[prior_z], hint_out = [hint_z])

    @torch.no_grad()
    def get_prior_hints(self, hint):
        self.anything_prior.to(self.device)
        hint = self.anything_prior(hint).detach()
        hint = einops.rearrange(hint, 'b h w c -> b c h w')

        # sorry about it, namely hint_out
        return dict(hint_out=[hint])
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            ...
            # hint-network
            hint_out = torch.cat(cond['hint_out'], 1)
            dino_guider = self.dino_guider(hint_out)
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, guider = dino_guider)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=True, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        c_cat, c_prior, c, hint_out = c["c_concat"][0][:N], c["prior_out"][0][:N], c["c_crossattn"][0][:N], c["hint_out"][0][:N]
        ...
        if sample:
            ...
            samples, z_denoise_row = self.sample_log(
                    cond={"c_concat": [c_cat], "c_crossattn": [c], 'c_prior': [c_prior], 'hint_out':[hint_out]},
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_full = {"c_concat": [uc_cat], "c_crossattn": [uc_cross], 'hint_out': [hint_out]}
    ...
    def configure_optimizers(self):
        ...
        params += list(self.dino_guider.parameters())
